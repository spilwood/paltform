...
import { Check, TreePine, TreeDeciduous, Ruler, CircleDot, Layers, Send, Leaf } from "lucide-react"
...
export function CustomOrderForm() {
  ...
  const getEstimatedPrice = () => {
    ...
    const thicknessFactor = formData.productType === "slice" ? formData.thickness[0] / 2 : formData.thickness[0] / 10

    const pricePerUnit = Math.round(basePrice * diameterFactor * thicknessFactor)
    return {
      ...
      <Card className="overflow-hidden">
        <CardContent className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="mt-3 max-w-md text-muted-foreground">
            Спасибо за интерес к нашей продукции! Мы свяжемся с вами в течение 24 часов для уточнения деталей заказа.
          </p>
          ...
          >
            <span
              className={`flex h-6 w-6 items-center justify-center rounded-full text-xs ${
                s < step ? "bg-primary-foreground text-primary" : s === step ? "bg-primary-foreground/20" : "bg-muted"
              }`}
            >
              {s < step ? <Check className="h-3 w-3" /> : s}
            </span>
            <span className="hidden sm:inline">{s === 1 ? "Параметры" : s === 2 ? "Контакты" : "Проверка"}</span>
          </button>
        ))}
      </div>
      ...
      <CardContent className="p-6 md:p-8">
        ...
        {step === 1 && (
          <div className="space-y-8">
            <div>
              <CardTitle className="text-xl">Выберите параметры изделия</CardTitle>
              <CardDescription className="mt-1">Укажите характеристики для индивидуального распила</CardDescription>
            </div>
            ...
            <div className="space-y-4">
              ...
              <RadioGroup
                ...
                onValueChange={(value: WoodType) => setFormData((prev) => ({ ...prev, woodType: value }))}
              ...
              >
                <Label
                  ...
                  className={`flex cursor-pointer flex-col items-center gap-3 rounded-xl border-2 p-6 transition-all hover:border-primary/50 ${
                    formData.woodType === "birch" ? "border-primary bg-primary/5" : "border-border"
                  }`}
                >
                  <TreeDeciduous
                  ...
                  <div className="text-center">
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">Светлая, выразительный рисунок</p>
                  </div>
                  <RadioGroupItem value="birch" id="birch" className="sr-only" />
                </Label>
                <Label
                  htmlFor="pine"
                  className={`flex cursor-pointer flex-col items-center gap-3 rounded-xl border-2 p-6 transition-all hover:border-primary/50 ${
                    formData.woodType === "pine" ? "border-primary bg-primary/5" : "border-border"
                  }`}
                >
                  <TreePine
                  ...
                  <div className="text-center">
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">Тёплый оттенок, смолистая</p>
                  </div>
            ...
            <div className="space-y-4">
              ...
              >
                <Label
                  htmlFor="slice"
                  className={`flex cursor-pointer flex-col items-center gap-3 rounded-xl border-2 p-6 transition-all hover:border-primary/50 ${
                    formData.productType === "slice" ? "border-primary bg-primary/5" : "border-border"
                  }`}
                >
                  <CircleDot
                  ...
                  <div className="text-center">
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">Плоский срез 1-5 см</p>
                  </div>
                  <RadioGroupItem value="slice" id="slice" className="sr-only" />
                </Label>
                <Label
                  htmlFor="stump"
                  className={`flex cursor-pointer flex-col items-center gap-3 rounded-xl border-2 p-6 transition-all hover:border-primary/50 ${
                    formData.productType === "stump" ? "border-primary bg-primary/5" : "border-border"
                  }`}
                >
                  <Layers
                  ...
                  <div className="text-center">
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">Высокий цилиндр 10-40 см</p>
                  </div>
                  <RadioGroupItem value="stump" id="stump" className="sr-only" />
                </Label>
              </RadioGroup>
            </div>
            ...
            <div className="space-y-6">
              <div className="space-y-4">
                ...
                <Slider
                  ...
                  onValueChange={(value) => setFormData((prev) => ({ ...prev, diameter: value }))}
                  min={5}
                  max={40}
                  step={1}
                  className="py-2"
              ...
              <div className="space-y-4">
                ...
                />
                <div className="flex justify-between text-xs text-muted-foreground">
                  <span>{formData.productType === "slice" ? "1 см" : "10 см"}</span>
                  <span>{formData.productType === "slice" ? "5 см" : "40 см"}</span>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <Select
                ...
                onValueChange={(value: ProcessingType) => setFormData((prev) => ({ ...prev, processing: value }))}
              >
            ...
            <div className="flex items-center justify-between rounded-lg border border-border p-4">
              <div>
                ...
                <p className="text-sm text-muted-foreground">Натуральный край с корой</p>
              </div>
              <Button
                variant={formData.barkPreserved ? "default" : "outline"}
                size="sm"
                onClick={() => setFormData((prev) => ({ ...prev, barkPreserved: !prev.barkPreserved }))}
              >
            ...
            <div className="rounded-xl bg-muted/50 p-6">
              <div className="flex items-center justify-between">
                <span className="text-muted-foreground">Ориентировочная цена за шт.</span>
                <span className="font-mono text-lg">{price.perUnit} ₽</span>
              </div>
              <div className="mt-2 flex items-center justify-between">
                <span className="font-medium">Итого за {formData.quantity} шт.</span>
                <span className="font-mono text-2xl font-semibold">{price.total.toLocaleString("ru-RU")} ₽</span>
              </div>
              <p className="mt-3 text-xs text-muted-foreground">
                * Окончательная цена будет рассчитана после обсуждения деталей заказа
              </p>
            </div>
        ...
        {step === 2 && (
          <div className="space-y-8">
            <div>
              ...
              <CardDescription className="mt-1">Заполните данные для связи с вами</CardDescription>
            </div>
            ...
            <Alert>
              <Leaf className="h-4 w-4" />
              <AlertDescription>
                Мы свяжемся с вами в течение 24 часов для обсуждения деталей заказа и финальной стоимости.
              </AlertDescription>
            </Alert>
        ...
        {step === 3 && (
          <div className="space-y-8">
            <div>
              ...
              <CardDescription className="mt-1">Убедитесь, что все данные указаны верно</CardDescription>
            </div>

            {/* Order Summary */}
            <div className="space-y-4">
              <div className="rounded-xl border border-border p-6">
                <h4 className="font-medium text-muted-foreground">Параметры изделия</h4>
                <dl className="mt-4 grid gap-3 text-sm">
                  <div className="flex justify-between">
                    <dt className="text-muted-foreground">Порода</dt>
                    <dd className="font-medium">{formData.woodType === "birch" ? "Берёза" : "Сосна"}</dd>
                  </div>
                  <div className="flex justify-between">
                    <dt className="text-muted-foreground">Тип</dt>
                    <dd className="font-medium">{formData.productType === "slice" ? "Спил" : "Пенёк"}</dd>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    <dt className="text-muted-foreground">{formData.productType === "slice" ? "Толщина" : "Высота"}</dt>
                  ...
                  <div className="flex justify-between">
                    ...
                    <dd className="font-medium">{formData.barkPreserved ? "Сохранить" : "Удалить"}</dd>
                  </div>
                </dl>
              </div>

              <div className="rounded-xl border border-border p-6">
                <h4 className="font-medium text-muted-foreground">Контактные данные</h4>
                <dl className="mt-4 grid gap-3 text-sm">
                  <div className="flex justify-between">
                  ...
                  {formData.comment && (
                    <div className="flex flex-col gap-1">
                      ...
                      <dd className="text-muted-foreground">{formData.comment}</dd>
                    </div>
                  )}
                </dl>
              </div>
