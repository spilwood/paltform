...
export function GallerySection() {
  ...
  const [isAutoPlaying, setIsAutoPlaying] = useState(false); // Disabled by default per AGENTS.md: avoid autoplay
  ...
  // Early return if no works available
  if (!currentWork) {
    return null;
  }
    ...
    <section className="py-20 md:py-28">
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="mt-3 max-w-xl text-sm sm:text-base text-muted-foreground">
              Уникальные изделия, созданные из наших спилов талантливыми
              мастерами
            </p>
          </div>
          <Button
        ...
        <div
          ...
          onMouseLeave={() => {}} // Don't auto-resume, let user control
          role="region"
          aria-roledescription="carousel"
          aria-label="Галерея работ мастеров"
        >
          <div className="overflow-hidden rounded-2xl bg-card">
            <div className="grid grid-cols-1 lg:grid-cols-2">
              <AspectRatio ratio={4 / 3} className="bg-muted">
                <Image
                  src={
                    ...
                    "/placeholder.svg?height=600&width=800&query=wooden craft handmade"
                  }
              ...
              <div className="flex flex-col justify-center p-6 sm:p-8 lg:p-12">
                ...
                <h3 className="mt-4 text-xl sm:text-2xl font-medium lg:text-3xl">
                  {currentWork.title}
                </h3>
                ...
                <div className="mt-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                  <span className="text-2xl font-semibold tabular-nums">
                    {currentWork.price.toLocaleString("ru-RU")}&nbsp;₽
                  </span>
                  <Button asChild className="w-full sm:w-auto">
                    <Link href={`/mastera/raboty/${currentWork.slug}`}>
                      Подробнее
                    </Link>
                  </Button>
                </div>
              ...
              >
                {works.map((work, index) => (
                  <button
                    ...
                    aria-label={`Слайд ${index + 1}: ${work.title}`}
